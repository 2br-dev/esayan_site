...
{foreach $app->getJs() as $js}
    ...
    <script type="{$js.params.type|default:"text/javascript"}" src="{$js.file}"{if $js.params.async} async{/if}{if $js.params.defer} defer{/if}{if $js.params.crossorigin} crossorigin="{$js.params.crossorigin}"{/if}></script>
...
<body {if $app->getBodyClass()!= ''}class="{$app->getBodyClass()}"{/if} {$app->getBodyAttrLine()}>
    ...
    {$js.params.before}<script type="{$js.params.type|default:"text/javascript"}" src="{$js.file}"{if $js.params.async} async{else} defer{/if}{if $js.params.crossorigin} crossorigin="{$js.params.crossorigin}"{/if}></script>{$js.params.after}
    {/foreach}
